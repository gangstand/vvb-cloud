Vue.createApp({
  ...
  data() {
    return {
      errors: { // Errors texts
        notSert: 'Архив должен быть в формате  .zip',
        sertNull: 'Выберите архив',
      ...
      isSert: false,
      messageNumber: 1,
      messages: ['', 'Выберите документ, который необходимо подписать', 'Выберите архив', 'Расшифровать']
    }
  },
  methods: {
    ...
    checkValidSert() {
      if(this.validators.isSertTouched) return  !(this.sertName.includes('.zip'))
    },
    checkNullSert() {
